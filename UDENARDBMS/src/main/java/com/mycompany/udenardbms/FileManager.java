...
public class FileManager {
    ...
    private HashMap<String, ArrayList<Object>> readAndWriteFile(String tbl, ArrayList<Schema> schemas, Integer predicion) {
        ...
        try (RandomAccessFile dis = new RandomAccessFile("classes.tbl", "rw");
             RandomAccessFile filetable = new RandomAccessFile(tbl + ".tbl", "rw");
             RandomAccessFile output = new RandomAccessFile("temp_" + tbl + ".tbl", "rw")) {
            ...
            if (filetable.length() > 0) {
                while (filetable.getFilePointer() < filetable.length()) {
                    ...
                    if (classe != predicion) {
                        ...
                        // Si la clase no coincide con la predicción, puedes eliminar la última fila escrita
                        // Si deseas eliminar del archivo, deberías manejar el archivo de manera temporal o sobrescribir
                    } else {
                        exitcolumdef = false;
                    }
                }
